...
const ChapterAccessForm = ({
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			...
			toast.error("Error al actualizar el título del capítulo");
		}
	};
	...
	return (
		<div className="mt-6 border bg-slate-100 rounded-md p-4">
			...
			{!isEditing ? (
				...
				>
					{initialData.isFree ? (
						<>El capítulo es gratuito</>
					) : (
						<>El capítulo no es gratuito</>
					)}
				</p>
			) : (
				<Form {...form}>
							...
							render={({ field }) => (
								<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
									...
									<div className="space-y-1 leading-none">
										<FormDescription>
											Marque esta casilla si desea que este capítulo sea
											gratuito
										</FormDescription>
									</div>
								</FormItem>
